...
export class LLMConnectionPrompter {
    ...
    async generateConnections(fileContent: string, sources: ChunkData[]): Promise<ConnectionOutput> {
        ...
        console.debug("promptTemplate", promptTemplate);

        console.debug("[generateConnections] Number of sources:", sources.length);
        
        if (sources.length === 0) {
            console.warn("[generateConnections] No sources found, returning empty result");
            return {
                editorial: [],
                connections: []
            };
